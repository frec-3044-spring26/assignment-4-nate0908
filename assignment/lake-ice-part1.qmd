...
ggplot(sunapee, aes(x = year, y = ice_off_doy))+
  geom_point()+
  geom_line()+
  labs(title = "Ice-off Day Lake Sunapee",
       x = "Year",
       y = "Ice-off Day")
```

# How does ice cover change over time at Lake Sunapee?
-Based on the graph there is no change in the y direction
...
-I would expect it to be negative based on the Earth progressively heating up more and more each year
...
- The direction of the slope is negative and it matches my prediction

**Answer 3:**

# Is our model any good?
- No because the current model does not show any change 
...
- The R2 for the regression is 0.2055. This indicates that year and ice_off_doy are not strongly related.The year itself does not dictate when the ice-off occurs.
...
sunapee <- sunapee %>% 
  mutate(
    ...
    ice_off_doy = as.numeric(as.character(ice_off_doy)),
    predicted = fit$coefficients[1] + year *fit$coefficients[2], 
         residual = ice_off_doy - predicted) 
#Observed - Predicted
```
...
ggplot(sunapee, aes(x=resid))+
  ...
  labs(
        title = "Residuals for Lake Sunapee Ice-off",
        x = "Residual",
        y = "Count")
```
...
- Using the equation 7/0.10602 we find that it takes around 66 years for the lake to experience a one week shift in the ice-off day. It is meaningful, but this is also happening over the course of 66 years, which lessens its impact in the short term.
**Answer 8:**
...
- I feel somewhat comfortable. I took Biological Statistics last fall so I am somewhat used to R and what it can do.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy))+
      ...
      geom_abline(
        ...
        color = "blue")+
      labs(
        title = "Ice-Off Day for Lake Sunapee",
        x = "Year",
        y = "Ice off Day")
```
...
resid <- quantile(sunapee$resid, probs = c(0.05, 0.95))
lower <- pred + resid[1]
upper <- pred + resid[2]
width <- upper - lower
...
Pre1970 <- lm(ice_off_doy ~ year, data = SPre1970)
Post1970 <- lm(ice_off_doy ~ year, data = SPost1970)
summary(Pre1970)
summary(Post1970)

plot <- ggplot(sunapee, aes(x = year, y = ice_off_doy))+
              geom_point()+
              geom_abline(
                intercept = coef(Pre1970)[1],
                slope = coef(Pre1970)[2],
                color = "blue")+
              geom_abline(
                intercept = coef(Post1970)[1],
                slope = coef(Post1970)[2],
                color = "red")+
              labs(
                title = "Lake Sunapee Ice-Off Day (Pre 1970 vs Post 1970)",
                x = "Year",
                y = "Ice-Off Day of Year")
plot
```
-Based on the plot, the ice-off day of year is changing more rapidly in recent years
